...
namespace IAUS.ECS2
{
    ...
    public class PatrolAction : SystemBase
    {
        ...
        protected override void OnCreate()
        {
            ...
            {
                All = new ComponentType[] { ComponentType.ReadWrite(typeof(Patrol)), ComponentType.ReadWrite(typeof(LocalToWorld)),
                    ComponentType.ReadOnly(typeof(BaseAI)),ComponentType.ReadOnly(typeof(PatrolActionTag)),ComponentType.ReadOnly(typeof(PatrolUpdateTag))
                },

            });
            _patrolUpdatesQueryLeader = GetEntityQuery(new EntityQueryDesc()
            {
                All = new ComponentType[] {  ComponentType.ReadWrite(typeof(SquadMemberBuffer))
                   ,ComponentType.ReadOnly(typeof(PatrolUpdateTag)) ,ComponentType.ReadWrite(typeof(LeaderTag)) }
        ...
        protected override void OnUpdate()
        {
            ...
            }.Schedule(_patrolUpdatesQueryLeader, systemDeps);
            _entityCommandBufferSystem.AddJobHandleForProducer(systemDeps);
       ...
       [BurstCompile]
        ...
        [BurstCompile]
        public struct UpdateLeaders : IJobChunk
        {
            ...
            public void Execute(ArchetypeChunk chunk, int chunkIndex, int firstEntityIndex)
            {
                ...
                for (int i = 0; i < chunk.Count; i++)
                {
                    DynamicBuffer<SquadMemberBuffer> SquadBuffer = squadBufferAccessor[i];
                    if (SquadBuffer.Length > 0)
                    {
                        for (int j = 0; j < SquadBuffer.Length; j++)
                        {
                            entityCommandBuffer.AddComponent<getpointTag>(chunkIndex, SquadBuffer[i].SquadMember);
